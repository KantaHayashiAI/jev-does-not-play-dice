...
- 28 Node tests and 9 Python tests, using mock API responses.
...
- API-free previews of all three example files, without installed npm dependencies.
- Relative documentation links, English/Japanese navigation, and MIT metadata.
- Recorded data, request snapshots, numeric summaries, examples, and figures match the previous reader package byte for byte.
...
The GitHub Actions workflow is configured to run offline tests, install the pinned SDK, and verify the `experimental_evaluate` export. The workflow itself has not been run here.
