...
| `data/recorded/repeat.json` | `jev_determinism_results.json` | 30 of 120 rows: `case == "uniform_die"` |
...
- **Repeat:** Contains `choice`, `top`, the distribution, and confidence for a fixed request. No hidden outcome was attached, so no accuracy is computed.
